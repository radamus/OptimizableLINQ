...
namespace OptimizableLINQBenchmark
{
    class NessosBench
    {
        ...
        {
            var rnd = new Random();
            var v = Enumerable.Range(1, 100000000).Select(x => rnd.NextDouble()).ToArray();
            ...
            Measure("Sum Linq", () => SumLinq(v),
                    "Sum Opt Compiled", SumLinqOptCompiled(v),
                    cmp);
            ...
            Measure("Sum of Squares Linq", () => SumSqLinq(v),
                    "Sum of Squares Opt Compiled", SumSqLinqOptCompiled(v),
                    cmp);
            ...
            Measure("Cartesian Linq", () => CartLinq(v1, v2),
                "Cartesian Opt Compiled", CartLinqOptCompiled(v1, v2),
                cmp);
        ...
        static Func<double> SumLinqOptCompiled(double[] values)
        {
            return values.AsQueryExpr().Sum().Compile();
        }
        ...
        static Func<double> SumSqLinqOptCompiled(double[] values)
        {
            return values.AsQueryExpr().Select(x => x * x).Sum().Compile();
        }
        ...
        static Func<double> CartLinqOptCompiled(double[] dim1, double[] dim2)
        {
            return (from x in dim1.AsQueryExpr()
                    from y in dim2
